...
const NAV_ITEMS = [
  { id: 'profile', path: '/dashboard/profile', label: 'Profile Details', shortLabel: 'Profile', icon: User },
  { id: 'orders', path: '/dashboard/orders', label: 'Order History', shortLabel: 'Orders', icon: Package },
  { id: 'wishlist', path: '/dashboard/wishlist', label: 'Wishlist', shortLabel: 'Wishlist', icon: Heart },
  { id: 'testimonials', path: '/dashboard/testimonials', label: 'My Testimonial', shortLabel: 'Testimonial', icon: MessageSquare },
  { id: 'settings', path: '/dashboard/settings', label: 'Settings', shortLabel: 'Settings', icon: Settings },
];

const navLinkClass = ({ isActive }: { isActive: boolean }) =>
  `flex items-center gap-3 px-4 py-3.5 rounded-xl font-label-md transition-all ${
    isActive
      ? 'bg-primary text-on-primary shadow-sm'
      : 'text-on-surface-variant hover:bg-surface-container hover:text-on-surface'
  }`;

const chipLinkClass = ({ isActive }: { isActive: boolean }) =>
  `inline-flex items-center gap-2 px-4 py-2 rounded-full font-label-md text-sm whitespace-nowrap transition-all shrink-0 ${
    isActive
      ? 'bg-primary text-on-primary shadow-sm'
...
export const DashboardLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-surface pt-8 pb-24 md:pb-16">
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row gap-6 md:gap-8">
            ...
            <div className="bg-surface-container-lowest rounded-3xl p-6 shadow-sm border border-outline-variant/30 mb-6">
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="font-headline-md text-on-surface truncate" title={displayName}>{displayName}</h3>
          ...
          <div className="md:hidden space-y-4">
            <div className="bg-surface-container-lowest rounded-3xl p-4 shadow-sm border border-outline-variant/30 flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-primary-container text-on-primary-container flex items-center justify-center font-headline-md text-xl shrink-0">
                {profile?.first_name?.[0] || user?.email[0].toUpperCase()}
              </div>
              <div className="min-w-0 flex-1">
                <h3 className="font-headline-md text-on-surface truncate" title={displayName}>{displayName}</h3>
            ...
            <nav className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-none">
              {NAV_ITEMS.map(({ path, shortLabel, icon: Icon }) => (
                <NavLink key={path} to={path} className={chipLinkClass}>
            ...
            <button
              onClick={logout}
              className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-error-container/50 text-error rounded-2xl shadow-sm border border-error/20 font-label-md hover:bg-error-container transition-colors"
            >
          ...
          <main className="flex-1 bg-surface-container-lowest rounded-3xl p-4 sm:p-8 shadow-sm border border-outline-variant/30 min-w-0">
            <Outlet />
          </main>
        </div>
      </div>
...
export const DashboardProfile = () => {
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-8 pb-4 border-b border-outline-variant/30 gap-4">
    ...
    >
      ...
      {ordersLoading ? (
        ...
        <div className="space-y-6">
          {orders.map((order) => (
            <div key={order.id} className="bg-surface hover:bg-surface-container-lowest transition-colors rounded-[2rem] p-4 sm:p-8 border border-outline-variant/30 shadow-sm">
                ...
                {order.items.slice(0, 3).map((item) => (
                  <div key={item.id} className="flex items-center gap-4 sm:gap-6">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-label-lg text-base sm:text-lg text-on-surface truncate mb-1">{item.product_name}</h4>
